...
# -*- coding: utf-8 -*-
"""
Lizbon + uzaktan is ilani takipcisi
...
Ortam degiskenleri (GitHub Secrets):
  ...
  CHAT_ID_IS  - Is ilanlarinin gidecegi Telegram sohbeti
"""
...
SITELER_DOSYASI = "siteler_is.txt"
...
KALIPLAR = [
    ("olx.pt", r"/d/anuncio/([\w\-]+?-ID\w+)\.html", "https://www.olx.pt/d/anuncio/{id}.html"),
    ("net-empregos.com", r"/(\d{5,9}/[\w\-]+)", "https://www.net-empregos.com/{id}/"),
    ("emprego.sapo.pt", r"/oferta/(\d+)", "https://emprego.sapo.pt/oferta/{id}"),
    ("itjobs.pt", r"/oferta/(\d+)", "https://www.itjobs.pt/oferta/{id}"),
    ("landing.jobs", r"/jobs/(\d+)", "https://landing.jobs/jobs/{id}"),
    ("randstad.pt", r"/empregos/([\w\-]+_[\w\-]+)", "https://www.randstad.pt/empregos/{id}"),
    ("workable.com", r"/view/([\w\-]+)", "https://jobs.workable.com/view/{id}"),
    ("remoteok.com", r"/remote-jobs/([\w\-]+)", "https://remoteok.com/remote-jobs/{id}"),
    ("weworkremotely.com", r"/remote-jobs/([\w\-]+)", "https://weworkremotely.com/remote-jobs/{id}"),
    ("remotive.com", r"/remote-jobs/([\w\-/]+\d{4,})", "https://remotive.com/remote-jobs/{id}"),
]

# Bilinmeyen siteler icin genel ilan linki tanima
GENEL_DESEN = (r"/((?:[\w\-]+/)*(?:oferta|ofertas|emprego|empregos|vaga|vagas|"
               r"job|jobs|career|careers|position|anuncio|vacancy)[\w\-]*/[\w\-]{8,})")
...
def ilan_mesaji(ad, link, metin, not_=None):
    satirlar = [f"<b>💼 Yeni ilan · {esc(ad)}</b>"]
    ozet = temizle(metin)
    if ozet:
...
def main():
    ...
    if not siteler:
        log("siteler_is.txt icinde aktif link yok.")
        return
    ...
    with sync_playwright() as p:
        ...
        baglam = tarayici.new_context(
            user_agent=TARAYICI_KIMLIGI, locale="en-GB",
            timezone_id="Europe/Lisbon", viewport={"width": 1366, "height": 900})
    ...
    if ANINDA_GONDER:
        ...
        if simdi().hour >= GONDERIM_SAATI and durum.get("son_gonderim") != bugun:
            ...
            telegram_gonder(
                f"💼 <b>Bugünün iş ilanları: {len(kuyruk)}</b>\n"
